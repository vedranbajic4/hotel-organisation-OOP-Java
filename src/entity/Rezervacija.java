...
public class Rezervacija {
	...
	public Rezervacija(TipSobe tipSobe, LocalDate datumDolaska, LocalDate datumOdlaska, LocalDate datumKreiranja) {
		this.id = ID_GENERATOR++;
		this.status = StatusRezervacije.NACEKANJU;
		this.tipSobe = tipSobe;
		this.datumDolaska = datumDolaska;
	...
	public Rezervacija(int id, StatusRezervacije status, TipSobe tipSobe, Soba soba, LocalDate datumDolaska, LocalDate datumOdlaska, float cena, LocalDate datumKreiranja) {
		if(id >= ID_GENERATOR) ID_GENERATOR = id+1;
		this.id = id;
        this.status = status;
        this.tipSobe = tipSobe;
	...
	public Rezervacija(int id, TipSobe tipSobe, LocalDate datumDolaska, LocalDate datumOdlaska, ArrayList<DodatnaUsluga> usluge, LocalDate datumKreiranja) {
		if(id >= ID_GENERATOR) ID_GENERATOR = id+1;
		this.id = id;
		this.status = StatusRezervacije.NACEKANJU;
        this.tipSobe = tipSobe;
	...
	public String[] forTable() {
		String sobaId = "-1";
		if(this.soba != null) sobaId = Integer.toString(this.soba.getId());
		String[] ret = { Integer.toString(id), status.name(), tipSobe.getBrojKreveta()+"", tipSobe.getRaspored(), sobaId,
				datumDolaska.toString(), datumOdlaska.toString(), Float.toString(cena) };
		return ret;
	}
